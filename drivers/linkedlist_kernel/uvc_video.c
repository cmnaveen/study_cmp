...
{ 
	...
	if((ret = usb_submit_urb(urb,GFP_ATOMIC))<0)
	{
        ...
        INIT_LIST_HEAD(node->queue_list);
...
{   //write it into a file
	...
	while (!kthread_should_stop()) {/*The thread cannot exit until the kthread_should_stop() function returns 1*/
		...
		else
		{
			...
			while((node=list_first_entry(head,struct queue,queue_list)) != NULL)
			{
				wait_event_interruptible(wq, flag == 1);
				flag =0;
				printk("\n%s: called from this LINE:%d\n", __func__,__LINE__);
				npkt = node->data_urb->number_of_packets;       
				for(i=0 ; i < npkt ; i++)
				{
					unsigned long int atemp=0,temp_len=1024;
					actual_len = node->data_urb->iso_frame_desc[i].actual_length;
					current_pos = node->data_urb->iso_frame_desc[i].offset;
					printk("\n%s: called from this LINE:%d\n", __func__,__LINE__);

					while((actual_len <=temp_len) || (temp_len != 0)){
						len = vfs_write(fp, node->data_urb->transfer_buffer+current_pos, temp_len, &fp->f_pos);
						if (len < 0) {
